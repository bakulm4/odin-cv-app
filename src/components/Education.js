...
//     }    
// }
export default function Education({educationData,editEducation,deleteEducation}) {
            ...
            {educationData.map((education,index)=>{
                return (
                    <section className='sub-section' key={index}>
                        ...
                        <div className="section-top-icon icons">
                            <CiEdit className='icon none' onClick={()=>editEducation(index)}/>
                            <BiTrash className='icon none' onClick={()=>deleteEducation(index)}/>
                        </div>
                    </section>
                );
            })}
        </div>
        <AiOutlinePlus className='section-top-icon icon none' onClick={()=>editEducation(null)}/>
    </div>
    )
    // }    
}

export function EducationEditor({educationData, handleEdit}) {


    const [education_data,setEducation_data] = useImmer(educationData?
        educationData:
        {
            school:null,
            degree:null,
    ...
    function canSaveData(){
        ...
        return education_data.school && education_data.school.length>0 &&
            education_data.degree && education_data.degree.length>0  &&
            education_data.graduationDate && education_data.graduationDate.length>0 &&
            education_data.location.city && education_data.location.city.length>0 &&
            education_data.location.state && education_data.location.state.length>0 &&
            education_data.achievements.length>0 
    }
    // render(){
    return  (
    <div className='editor-container'>
        ...
        <div className='editor-content education-editor-content'>
            <div className='school'>
                <label htmlFor='school'>School</label>
                <input type='text' id='school' onChange={(e)=> setEducation_data(draft=>{draft.school=e.target.value})} value={education_data.school?education_data.school:''} placeholder='School A'></input>
            </div>
            <div className='degree'>
                <label htmlFor='degree'>Degree or Field of Study</label>
                <input type='text' id='degree' onChange={(e)=> setEducation_data(draft=>{draft.degree=e.target.value})} value={education_data.degree? education_data.degree: ''} placeholder='Bachelor of Business Administration'></input>
            </div>
            <div className='grad-date'>
                <label htmlFor='grad-date'>Graduation Date</label>
                <input type='text' id='grad-date' 
                    onChange={(e)=> setEducation_data(draft=>{draft.graduationDate=e.target.value})}
                    value={education_data.graduationDate?education_data.graduationDate:''}
                    placeholder='MM/YYYY' pattern='[0-1]\d/\d{4}'
                    >
                </input>
            </div>
            <div className='city'>
                <label htmlFor='city'>City</label>
                <input type='text' id='city' onChange={(e)=> setEducation_data(draft=>{ draft.location.city=e.target.value})} value={education_data.location.city?education_data.location.city:''} placeholder='New York'></input>
            </div>
            <div className='state'>
                <label htmlFor='state'>State</label>
                <input type='text' id='state' onChange={(e)=> setEducation_data(draft=>{ draft.location.state=e.target.value})} value={education_data.location.state?education_data.location.state:''} placeholder='NY'></input>
            </div>
            <div className='achievements list'>
                <label htmlFor='achievements'>What were your academic achievements?</label>
                <ul id='achievements'>
                    {education_data.achievements.map((achievement,i)=>
                        <li key={i}>
                            <input type='text'
                                onChange={(e)=> 
                                    setEducation_data(draft=>{ draft.achievements= draft.achievements.map((achievement,index)=> index===i?e.target.value:achievement)     
                                        })} 
                                value={education_data.achievements[i]}
                                placeholder='Add academic achievement'
                            >
                            </input>
                            <GrClose className='delete' onClick={()=> setEducation_data(draft=>{ draft.achievements= draft.achievements.filter((achievement,index)=> index!==i) })}/>
                        </li>
                        )}
                </ul>
                <AiOutlinePlus className='add' onClick={()=>setEducation_data(draft=>{ draft.achievements= draft.achievements.concat('')})}/>
                </div>

        </div>
        <div className='editor-footer'>
            <button className='btn' onClick={()=>handleEdit(false,null)}>Cancel</button>
            <button className='btn save' disabled={!canSaveData()} onClick={()=>handleEdit(true,education_data)}>Save</button>
        </div>
    </div>
    )
    // }    
